        ...
        <div class="list_items">
            <div v-for="method in methods.list" :key="method.id" class="items">
                ...
                <div v-if="OrdersMethod == method.id" class="operations">
                    <span v-for="item in method.payments_purchases" :key="item.id" class="items">
                        <div>{{item.id}}</div>
                        <div>{{item.purchase.user.surname}}</div>
                        <div>{{item.purchase.supplier?.surname}}</div>
...
async function getClients() {
    await axios.get('purchur/getPayments')
    .then((response) => {
